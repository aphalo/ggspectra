...
| `stat_find_qty` | point (highlight qty at wl)  | wavelength label, spectral quantity label | 
...
Two stats are available for highlighting arbitrary locations in spectra, one of them, `stat_find_wls()` accepts a target for the spectral quantity and locates the corresponding wavelength values while the other, `stat_find_qty()` accepts a target wavelength value and locates the corresponding spectral quantity value. Stats `stat_find_wls()` and `stat_find_qty()` subset the original data or generate new data by interpolation.
...
The examples that follow, apply with minimal changes to `stat_peaks()`, `stat_valleys()`, `stat_find_wls()` and `stat_find_qty()`.
...
Stats `stat_find_wls()` and `stat_find_qty()` are another complementary pair. The default target, used in this example, is the half maximum. 
