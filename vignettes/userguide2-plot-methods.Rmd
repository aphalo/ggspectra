...
and subtitles. The `"title"` annotation takes optional modifiers that can be used to specify the contents of the title and subtitle. These modifiers are: `objt`, `what`, `when`, `where`, `none`. For example `"title:objt"`, the default, adds a title with the name of the object being plotted. `what`, `when` and `where` use these three attributes if available. For example `"title:what:when" will use the `what.measured` attribute for the title and the `when.measured` attribute for the subtitle. 
