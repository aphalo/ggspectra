...
It is possible to animate ggplots created with `autoplot()` methods. Currently
`by.group = TRUE` has to be added to the call. This is because annotations need
to be added by group instead of by panel, so that they are present in all
frames. (Internally `by.group = TRUE` is passed to the statistics that support
it).

All label definitions created by functions and methods from 'ggspectra' (>=
0.3.16) are static or defined using `bquote()` as required by 'gganimate'.
...
**Currently, `label.qty = "total"` and `label.qty = "irrad"` trigger an ERROR for photon units when attempting to animate a plot created with `autoplot()` when `unit.out` is set through an R option. It does work when the same attempted by passing an argument in the call as above.**
