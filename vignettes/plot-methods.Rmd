...
```
The annotations `"peak.labels"` and `"valley.labels"` overridde `"peaks"` and `"valleys"`. They use the _repulsive_ geometry `geom_label_repel` from package 'ggrepel'.
```{r}
...
If one really needs to, one can add a suitable _stat_ using 'local' data. A peak annotaion could be added manually.
