...
| `stat_wl_strip` |  rect (color of wavelength or waveband) | text (label with waveband name) |
| `stat_wb_box` |  rect (color of waveband) | text (label with waveband name) |
| `stat_wb_column` |  rect (color of waveband) | text (label with waveband name) |
...
ggplot(sun.spct) + geom_line() + 
  stat_label_peaks(span = 31, geom = "label_repel", aes(label = ..y.label..), 
             label.fmt = "y = %1.2f", segment.colour = "black") +
  scale_fill_identity() + scale_color_identity()
```
...
```{r}
ggplot(sun.spct) +
  stat_wb_column(w.band = c(400,500), alpha = 0.5) +
  stat_wb_mean(w.band = c(400,500)) +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +
  theme_bw()
...
ggplot(yellow_gel.spct) +
  stat_wb_column(w.band = Plant_bands()) +
  stat_wb_mean(w.band = Plant_bands()[1:3], angle = 90, hjust = 0, 
               ypos.fixed = 0.1, color = "black",
               aes(label = paste(..wb.name.., " = ", ..y.label.., sep = ""))) +
  stat_wb_mean(w.band = Plant_bands()[4:6], angle = 90, hjust = 0, ypos.fixed = 0.1, 
                 aes(label = paste(..wb.name.., " = ", ..y.label.., sep = ""))) +  geom_line() + 
  scale_fill_identity() + scale_color_identity() +
  theme_bw()
```
...
```{r}
ggplot(yellow_gel.spct) +
  stat_wb_box(w.band = Plant_bands(), color = "white", ypos.fixed = 1) +
...
```{r}
ggplot(sun.spct) +
  stat_wb_column(w.band = waveband(CIE()), alpha = 0.5) +
...
ggplot(sun.spct) +
  ...
  stat_wb_e_sirrad(w.band = VIS_bands(), angle = 90,
...
my.bands <- split_bands(c(300,800), length.out = 10)
ggplot(sun.spct, unit.out = "photon") +
  stat_wb_column(w.band = my.bands, alpha = 0.7) +
  stat_wb_q_sirrad(w.band = my.bands, angle = 90, size = 2.5) +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +  
  theme_bw()
