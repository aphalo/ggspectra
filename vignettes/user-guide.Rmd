...
The _stat_s in previous sections can be used for unweighted irradiances, but not for BSWFs. These `irrad` _stat_s have additional formal parameters for passing metadata, and use method `photobiology::irrad()` internaly on a `photobiology::source_spct` object constructed from the
data for $x$ and $y$ _aesthetics_. Function `stat_wb_sirrad()` is equivalent to `stat_wb_mean()` and function `stat_wb_irrad()` is equivalent to `stat_wb_total()` following the same convention naming as in package `photobiology`.
...
ggplot(sun.spct, unit.out = "photon") +
  ...
  stat_wb_sirrad(w.band = PAR(), geom = "text",
                unit.in = "photon", time.unit = "second", 
                mapping = aes(label = sprintf("%s = %.3g", ..wb.name.., ..ymean.. * 1e6)), 
                ypos.mult = -0.5) +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +  
...
ggplot(sun.spct) +
  stat_wb_sirrad(w.band = VIS_bands(), geom = "rect", alpha = 0.5,
                unit.in = "energy", time.unit = "second") +
  stat_wb_irrad(w.band = VIS_bands(), angle = 90, geom = "text",
                unit.in = "energy", time.unit = "second", ypos.fixed = 0.05, hjust = 0,
                 aes(label = paste(..wb.name.., ..y.label.., sep = " = "),
                     color = "black")) +
  geom_line() + 
...
The examples for `stat_wb_mean` can be simplified by use of `wb_guide()` which adds both the statistics and the scale, and provides a default _mapping_ for both `ymin` and `ymax`. Furthermore, this function adds two layers using `"stat_wb_mean"`, one with _geom_ `"rect"` and one with _geom_ `"text"`. Another important difference is that these _geoms_ cannot be set by the user.
