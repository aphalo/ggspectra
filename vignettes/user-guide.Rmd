...
ggplot(sun.spct) +
  ...
  stat_wb_irrad(w.band = PAR(), vjust = 1.2, geom = "text",
                unit.in = "energy", time.unit = "second") +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +  
...
ggplot(sun.spct) +
  stat_wb_irrad(w.band = CIE(), geom = "rect", alpha = 0.5,
                unit.in = "energy", time.unit = "second") +
  stat_wb_irrad(w.band = CIE(), vjust = 1.2, geom = "text",
                unit.in = "energy", time.unit = "second") +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +  
...
ggplot(sun.spct) +
  ...
  stat_wb_irrad(w.band = VIS_bands(), angle = 90, geom = "text",
                unit.in = "energy", time.unit = "second", y.position = 0.05, hjust = 0,
                 aes(label = paste(..wb.name.., ..y.label.., sep = " = "),
                     color = "black")) +
  geom_line() + 
  scale_color_identity() + 
...
ggplot(sun.spct) + 
  geom_spct() + 
  irrad_guide(w.band = Plant_bands(), time.unit = "second", unit.in = "energy")
```

```{r}
ggplot(sun.spct) + 
  geom_spct() + 
  irrad_guide(w.band = CIE(), time.unit = "second", unit.in = "energy")
```

```{r, eval=FALSE}
ggplot(sun.daily.spct) + 
  geom_spct() + 
  irrad_guide(w.band = CIE(), time.unit = "day", unit.in = "energy")
```

```{r, eval=FALSE}
ggplot(sun.spct, unit.out = "photon") + 
  geom_spct() + 
  irrad_guide(w.band = PAR(), time.unit = "second", unit.in = "photon", label.mult = 1e6)
```
