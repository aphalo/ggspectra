---
title: "`ggspectra` `r packageVersion('ggspectra')`<br>User Guide"
author: "Pedro J. Aphalo"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{User Guide}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

## Introduction

```{r}
library(ggplot2)
library(photobiology)
library(photobiologyWavebands)
library(ggspectra)
```

```{r setup, include = FALSE, eval = FALSE}
library(svglite)
knitr::opts_chunk$set(
  dev = "svglite",
  fig.ext = ".svg"
)
```

```{r}
library(knitr)
opts_chunk$set(fig.path = 'figure/pos-', fig.align = 'center', fig.show = 'hold',
               fig.width = 7, fig.height = 4)
options(warnPartialMatchArgs = FALSE)
```

```{r}
two_suns.spct <- rbindspct(list(sun1 = sun.spct, sun2 = sun.spct * 2))
```

## ggplot() methods for spectra

```{r}
ggplot(sun.spct) + geom_line()
```

```{r}
ggplot(two_suns.spct) + aes(color = spct.idx) + geom_line()
```

```{r}
ggplot(sun.spct, unit.out = "photon") + geom_line()
```

```{r}
ggplot(yellow_gel.spct) + geom_line()
```

```{r}
ggplot(yellow_gel.spct, plot.qty = "absorbance") + geom_line()
```

## Stats


### Peaks and valleys

```{r}
ggplot(sun.spct) + geom_line() + stat_peaks()
```

```{r}
ggplot(sun.spct, unit.out = "photon") + geom_line() + stat_peaks()
```

```{r}
ggplot(sun.spct) + geom_line() + stat_valleys()
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(shape = 21) + scale_fill_identity()
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(shape = 21, span = 25, size = 2) + scale_fill_identity() +
  stat_peaks(geom = "label", span = 25, vjust = "bottom", size = 3, 
             color = "white", label.fmt = "%3.0f")
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(span = NULL, geom = "vline", linetype = "dotted") +
  stat_peaks(span = NULL, geom = "hline", linetype = "dotted")
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(span = 21, shape = 4, color = "red", size = 2)
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(span = 21, geom = "text")
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(span = 21, geom = "text", aes(label = ..y.label..))
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(span = NULL) +
  stat_peaks(span = NULL, geom = "text", vjust = -0.5, size = 2.5,
             y.label.fmt = "%.3f",
             aes(label = paste(..y.label.., "at", ..x.label.. , "nm")))
```

```{r}
ggplot(sun.spct) + geom_line() + 
  stat_peaks(span = 21, geom = "point", colour = "red") +
  stat_valleys(span = 21, geom = "point", colour = "blue") +
  stat_peaks(span = 51, geom = "text", colour = "red", 
             vjust = -0.3, label.fmt = "%3.0f nm") +
  stat_valleys(span = 51, geom = "text", colour = "blue", 
               vjust = 1.2, label.fmt = "%3.0f nm")
```

```{r}
ggplot(two_suns.spct) + aes(color = spct.idx) +
  geom_line() + ylim(NA, 1.8) +
  stat_peaks(span = NULL, color = "black") +
  stat_peaks(span = NULL, geom = "text", vjust = -0.5, size = 3,
             y.label.fmt = "%.3f", color = "black", 
             aes(label = paste(..y.label.., "at", ..x.label.. , "nm"))) +
  facet_grid(spct.idx~.)
```

## Color from wavelength

```{r}
ggplot(sun.spct) + 
  geom_line() +
  stat_color() + scale_color_identity()
```

```{r}
ggplot(sun.spct) +
  geom_line() +
  stat_color(shape = 21, color = "black") + 
  scale_fill_identity()
```

```{r}
ggplot(sun.spct) + 
  stat_color(geom = "bar") + 
  geom_point(shape = 21, color = "black", stroke = 1.2, fill = "white") +
  scale_fill_identity() + 
  scale_color_identity() + 
  theme_bw()
```

```{r}
ggplot(two_suns.spct) + aes(shape = spct.idx) +
  stat_color() + scale_color_identity() +
  geom_line() + 
  facet_grid(spct.idx~., scales = "free_y")
```

## Averages and similar summaries

```{r}
ggplot(sun.spct) + geom_line() + stat_average()
```

```{r}
ggplot(sun.spct) + 
  geom_line() +
  stat_average(range = c(300,500), label.fmt = "%.3f", color = "red")
```

```{r}
ggplot(sun.spct) +
  geom_line() + 
  stat_average(geom = "hline", color = "red") +
  stat_average(label.fmt = "Mean = %.3f", color = "red", vjust = -0.3)
```

```{r}
ggplot(two_suns.spct) + aes(color = spct.idx) +
  geom_line() + 
  stat_average(geom = "hline") +
  stat_average(label.fmt = "Mean = %.3f", vjust = -0.3, show.legend = FALSE)
```

```{r}
ggplot(sun.spct) +
  stat_average(range = c(400,500), geom = "rect", alpha = 0.2, fill = color(450)) +
  stat_average(range = c(400,500), label.fmt = "Mean = %.3f", vjust = -0.3, geom = "text") + 
  geom_line()
```

```{r}
ggplot(two_suns.spct) + aes(color = spct.idx) +
  geom_line() + 
  stat_average(geom = "hline") +
  stat_average(label.fmt = "Mean = %.3f", vjust = 1.2, show.legend = FALSE) +
  facet_grid(spct.idx~.)
```

```{r}
ggplot(two_suns.spct) + aes(color = spct.idx) +
  geom_line() + 
  stat_average(geom = "hline") +
  stat_average(label.fmt = "Mean = %.3f", vjust = 1.2, show.legend = FALSE) +
  facet_grid(spct.idx~., scales = "free_y")
```

### Wavebands

```{r}
ggplot(sun.spct) +
  stat_waveband(w.band = PAR(), geom = "rect", alpha = 0.5) +
  stat_waveband(w.band = PAR(), vjust = 1.2, geom = "text") +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +  
  theme_bw()
```

```{r}
ggplot(sun.spct) +
  stat_waveband(w.band = c(400,500), geom = "rect", alpha = 0.5) +
  stat_waveband(w.band = c(400,500), geom = "text", y.multiplier = 0.5) +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +
  theme_bw()
```

```{r}
ggplot(sun.spct) +
  stat_waveband(w.band = VIS_bands(), geom = "rect", alpha = 0.7) +
  stat_waveband(w.band = VIS_bands(), angle = 90, hjust = -0.1, geom = "text", 
                 aes(label = paste(..wb.name.., " = ", ..y.label.., sep = ""),
                     color = ..wb.color..)) +
  geom_line() + 
  scale_color_identity() + 
  scale_fill_identity() +
  ylim(NA, 1.1) + 
  theme_bw()
```

## Color guide

```{r}
ggplot(sun.spct) +
  geom_line() + 
  stat_color_guide(w.band = VIS_bands(), ymin = -0.03, ymax = 0) + 
  scale_fill_identity() +
  theme_bw()
```
 
which can be simplified by use of `color_guide()` which adds both the statistics and the scale. We addionaly leave `ymin` at its default of `-Inf` so that it is clipped at the edge of the plotting area.

```{r}
ggplot(sun.spct) +
  geom_line() + 
  color_guide(w.band = VIS_bands(), ymax = -0.025) + 
  theme_bw()
```

```{r}
ggplot(sun.spct) +
  geom_line() +
  color_guide(ymax = -0.025) + 
  theme_bw()
```

```{r}
ggplot(sun.spct) +
  color_guide(alpha = 0.4) + 
  geom_line() +
  theme_bw()
```

## Multiple stats in the same plot

```{r}
ggplot(sun.spct) +
  stat_color_guide(alpha = 0.4, ymin = -Inf, ymax = Inf) + 
  stat_waveband(w.band = PAR(), ymin = -0.01, ymax = 0, color = "black", geom = "rect") +
  stat_waveband(w.band = PAR(), integral.fun = "total",
                vjust = -0.5, geom = "text", color = "black",
                y.position = 0,
                aes(label = paste(..wb.name.., " = ", ..y.label.., sep = ""))) +  
  geom_line() + 
  scale_fill_identity() +
  theme_bw()
```

## Geometries

```{r}
ggplot(sun.spct) + geom_spct()
```

```{r}
ggplot(sun.spct) + 
  geom_spct(fill = color(sun.spct))
```

## Packaged stats and scales

### color_guide

```{r}
ggplot(sun.spct) + 
  color_guide(ymin = -0.05, ymax = 0) +
  geom_spct() 
```

```{r}
ggplot(sun.spct) + 
  color_guide() +
  geom_spct(alpha = 0.75, colour = "white", size = 1) 
```

```{r}
ggplot(sun.spct) + 
  color_guide() +
  geom_line(size = 2, colour = "white") +
  geom_line(size = 1, colour = "black") +
  geom_hline(yintercept = 0, colour = "grey92") +
  theme_bw()
```

### waveband_guide

```{r}
ggplot(sun.spct) + 
  geom_spct() + 
  waveband_guide(w.band = Plant_bands())
```

```{r}
ggplot(sun.spct) + 
  geom_spct() + 
  waveband_guide(w.band = Plant_bands(), guide.position = "top") +
  lims(y = c(NA, 1.1))
```

```{r}
ggplot(sun.spct) + 
  geom_spct() + 
  waveband_guide(w.band = Plant_bands(), guide.position = "top",
                 aes(label = sprintf("%s", ..wb.name..))) +
  ylim(NA, 1.1) + theme_bw()
```

```{r}
ggplot(sun.spct) + 
  geom_spct() + 
  waveband_guide(w.band = Plant_bands(), guide.position = "top",
                 aes(label = paste(..wb.name.., ..y.label.., sep = ": "))) +
  ylim(NA, 1.1)
```

```{r}
ggplot(sun.spct) + 
  geom_spct() + 
  waveband_guide(w.band = Plant_bands(), guide.position = "top2",
                 aes(label = paste(..wb.name.., ..y.label.., sep = "\n"))) +
  ylim(NA, 1.1)
```

## Plotting spectra

```{r}
plot(sun.spct)
```

```{r}
plot(sun.spct, label.qty = "mean")
```

```{r}
plot(sun.spct, label.qty = "contribution")
```

```{r}
plot(sun.spct, unit.out = "photon")
```

```{r}
plot(sun.spct, annotations = c("segments", "labels", "color.guide"))
```

```{r}
plot(sun.spct, 
     annotations = c("segments", "labels", "summaries", "color.guide"))
```

```{r}
plot(sun.spct, range = VIS())
```

```{r}
plot(sun.spct, w.band = PAR())
```

```{r}
plot(sun.spct, w.band = CIE())
```

```{r}
plot(sun.daily.spct)
```

```{r}
plot(two_suns.spct) + facet_wrap(~spct.idx)
```

```{r}
plot(two_suns.spct) + aes(linetype = spct.idx)
```

```{r}
plot(yellow_gel.spct, annotations = "colour.guide")
```

```{r}
plot(sun.spct) + geom_spct(fill = color(sun.spct)) + 
  geom_spct(data = yellow_gel.spct * sun.spct, color = "black", 
            fill = color(yellow_gel.spct * sun.spct))
```
