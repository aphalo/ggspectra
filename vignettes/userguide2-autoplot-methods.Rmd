...
in package 'photobiology' are available. Function `spct_classes()` lists these
classes.
...
use unless without the possibility to adjust the design and components of the
_ggplot_ object created. One possibility is to use the grammar of graphics to
add to the plot. However, it is also possible to control many features of the
plot by passing additional arguments in the call to `autoplot()`.

When plotting `source_spct` and `response_spct` objects we can change the basis of expression of spectral irradiance from `"energy"` to `"photon"`. Be aware that by design, all values, summaries and spectral data will always use the same base of expression.
...
In addition to the `autoplot()` methods for individual spectra, there are
`autoplot()` methods available for collections of spectra. Collections of
spectra like `two_suns.mspct` can be plotted also automatically, as they are
combined on-the-fly and plotted as shown above in the example for
`two_suns.spct`.

We use in the examples a simple collection of two spectra. The output and
...
similar. Function `mspct_classes()` lists them.
...
Above we showed the default faceting by passing `facets = TRUE`. We can set the number of columns by passing a number. For panels in one column we pass `facets = 1`.
...
All plotting of collections of spectra shown above were done with the default
...
With `"boundaries"`, we adds horizontal dashed line(s) showing the valid range of values. It does not override any other annotation. (The boundary line is shown in red when a plot displays out-of-range spectral data.)
