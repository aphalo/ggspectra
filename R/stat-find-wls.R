...
#'
#' \code{stat_find_wls} finds at which x positions values equal to a
#' target are located.
#'
...
#' \describe{
#'   \item{x}{x-value at or nearest to the match to the target as numeric}
#'   \item{y}{target value or y-value nearest to the target as numeric}
#'   \item{x.label}{x-value at or nearest to the match formatted as character}
#'   \item{y.label}{target value or y-value nearest to the target formatted as character}
...
#' ggplot(yellow_gel.spct) + geom_line() + stat_find_wls(target = 0.75)
...
StatFindWls <-
  ggplot2::ggproto("StatFindWls", ggplot2::Stat,
                   compute_group = function(data,
                                            ...
                                            x.label.fmt,
                                            y.label.fmt) {
                     wls.df <- photobiology::find_wls(data,
                                                      .fun = `<=`,
                                                      target,
                                                      interpolate,
                                                      col.name.x = "x",
...
#' library(ggplot2)
#' # ggplot() methods for spectral objects set a default mapping for x and y.
#' ggplot(yellow_gel.spct) + geom_line() + stat_find_qty(target = "half.range")
#' ggplot(yellow_gel.spct) + geom_line() +
...
StatFindQty <-
  ggplot2::ggproto("StatFindQty", ggplot2::Stat,
                   compute_group = function(data,
                     ...
                     # This is a bit of computational overkill for spectra
                     qty.df <- photobiology::find_wls(data,
                                                      .fun = `<=`,
                                                      target,
                                                      interpolate,
                                                      col.name.x = "y",
